 ...
 * and prunes snapshots older than 14 days.
 *
...
async function collectModel(
  ...
  fetchedAt: string,
): Promise<{ totalRows: number; errors: number; snowyIds: Set<number> }> {
  let totalRows = 0;
  let errors    = 0;
  ...
  return { totalRows, errors, snowyIds };
}
...
async function main() {
  ...
  const gfs = await collectModel('forecast', 'open-meteo', batches, fetchedAt);
  console.log(`[collect] open-meteo done — ${gfs.totalRows} rows, ${gfs.errors} errors`);

  // ── Open-Meteo ECMWF (commercial only) ────────────────────────────────────
  let ecmwf = { totalRows: 0, errors: 0, snowyIds: new Set<number>() };
  if (process.env.OPEN_METEO_KEY) {
    ecmwf = await collectModel('ecmwf', 'ecmwf', batches, fetchedAt);
  ...
  console.log(`[collect] done — ${totalRows} total rows`);
}
